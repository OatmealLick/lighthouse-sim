...
public class InternalRepository implements SimulationStateRepository
{
    ...
    {
        return RouteBuilder.builder().withRoad(one, two).withRoad(two, three).withRoad(three, one).build();
    }
    ...
    {
        return ImmutableSet.of(camera);
    }

    @Override
    public Set<MovementSensor> getMovementSensors()
    {
        return ImmutableSet.of(movementSensor);
    }
